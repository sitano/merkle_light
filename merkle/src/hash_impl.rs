...
macro_rules! impl_write {
    ($(($ty:ident, $meth:ident),)*) => {$(
        impl<H: Hasher> Hashable<H> for $ty {
            ...
            fn hash_slice_state(data: &[$ty], state: &mut H) {
...
impl<H: Hasher> Hashable<H> for bool {
    fn hash_state(&self, state: &mut H) { state.write_u8(*self as u8) }
}
...
macro_rules! impl_hash_tuple {
    () => (
        impl<H: Hasher> Hashable<H> for () {
            fn hash_state(&self, _state: &mut H) {}
        }
    );
    ...
    ( $($name:ident)+) => (
        impl<Z: Hasher, $($name: Hashable<Z>),*> Hashable<Z> for ($($name,)*) where last_type!($($name,)+): ?Sized {
            ...
            fn hash_state(&self, state: &mut Z) {
                let ($(ref $name,)*) = *self;
                $($name.hash_state(state);)*
            }
        }
    );
}
...
impl<H: Hasher, T: Hashable<H>> Hashable<H> for [T] {
    fn hash_state(&self, state: &mut H) {
        self.len().hash_state(state);
        Hashable::hash_slice_state(self, state)
    }
}
